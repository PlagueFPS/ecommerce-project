...
const ProductDetails = ({ product, products }) => {
  ...
  const { increaseQuantity, decreaseQuantity, quantity, onAdd, setShowCart, productStock, setProductStock } = useStateContext()
  const [index, setIndex] = useState(0)

  useEffect(() => {
    setProductStock(product.stock)
  }, [product.stock, setProductStock])
    ...
    <div>
      <div className="product-detail-container">
        <div>
          ...
          <h4>Details: </h4>
          <p>{ details }</p>
          <p className={ productStock <= 10 ? 'stock' : 'hide' }>
